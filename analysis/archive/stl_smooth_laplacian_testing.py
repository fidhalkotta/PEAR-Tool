...
def smooth_and_visualize(mesh, iterations, lambdas):
    ...
    for i, iteration in enumerate(iterations):
        for j, lambda_filter in enumerate(lambdas):
            # Apply Laplacian smoothing with the current set of parameters
            smoothed_mesh = mesh.filter_smooth_laplacian(number_of_iterations=iteration, lambda_filter=lambda_filter)

            smoothed_mesh = repair_mesh(smoothed_mesh)

            # Translate the mesh to its visualization position
            translation = translation_axis * translation_distance * (i * len(lambdas) + j+1)
...
iterations = [10]  # Example: 5, 10, 15 iterations
lambdas = [0.5, 1.0, 1.5]  # Example: 0.5, 1.0, 1.5 lambda_filter values
# lambdas = [0.5]

# Apply smoothing with different parameters and visualize
smooth_and_visualize(mesh, iterations, lambdas)
